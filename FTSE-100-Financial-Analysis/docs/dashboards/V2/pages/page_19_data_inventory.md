...
> Inventory of datasets and tables: grain, row counts, freshness, owner, and contract status.
...
1. What datasets exist and where are they located?
2. How fresh is each dataset?
3. Which datasets are critical to dashboards and which are optional?
...
|------|------|--------|---------|----------|
| mart.data_inventory | run | Curated mart | every 5 minutes (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.data_inventory | run | Curated mart | every 5 minutes (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.data_usage_map | run | Curated mart | every 5 minutes (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
...
|-----|------------|-------------|------------------|------|
| Datasets | Number of datasets tracked | count(rows) | Info | Coverage indicator |
| Stale Datasets | Datasets beyond freshness SLA | count(stale) | 0 preferred | Trigger investigation |
| Largest Table | Max rowcount table | argmax(rowcount) | Info | Capacity planning |
...
- **Measure 01:** `m_freshness_minutes`  
  - **Definition:** Freshness calculation  
  - **Implementation:** SQL/Python  
  - **Formula:**  
    ```text
    freshness = now - max(timestamp)
    ```

- **Measure 02:** `m_contract_status`  
  - **Definition:** Data contract status  
  - **Implementation:** Docs  
  - **Formula:**  
    ```text
...
|-----------|------|-------|----------|---------|------------|
| V1 | Table | Dataset Inventory | mart.data_inventory | Catalogue | Search + filter |
| V2 | Bar | Freshness by dataset | mart.data_inventory | Freshness overview | Hover |
| V3 | Table | Dataset Usage (criticality) | mart.data_usage_map | What matters | Filter |
...
- **Cross-page filter behaviour:** Cross-page filters: enabled (dataset selection)
...
**Latency notes:** Inventory supports ops readiness and trust
...
- `docs/dashboards/V2/exports/v2_page_19_data_inventory.png`
...
**Data inventory:** shows dataset coverage, grain, owner, retention, and freshness expectations.
This is a strong ‘data engineering maturity’ signal.

---

## 18) Data dictionary excerpt (minimum viable schema)
> This excerpt is the **minimum** required to render V2-P19. Full contracts live in `DATA_CONTRACTS.md`.

| Field | Type | Example | Rules / Notes |
|------|------|---------|---------------|
| dataset_name | string | mart.market_overview | Name |
| layer | string | mart | raw/staged/curated/mart |
| grain | string | timestamp×interval | Grain |
| owner | string | Reena | Owner |
| refresh | string | 5m in session | Refresh policy |
| retention | string | 90d intraday | Retention policy |
| contract_status | string | approved | draft/approved/deprecated |
| dq_score_latest | float | 98.0 | 0–100 |
| run_id_latest | string | 2026-02-16T10:05Z_02 | Latest run |


---

## 19) Tooltip, formatting & interaction spec (make it feel “real-time”)
- Inventory tooltip: dataset, grain, refresh, retention, contract status, latest dq/freshness.
...
- Dataset renamed → treat as breaking change and add alias mapping.

---

## 21) Validation & reconciliation tests (must pass before 4K export)
- Inventory list matches actual folder/table list used in dashboards.

---

## 22) Implementation notes (Power BI / Python / Streamlit)
- Maintain inventory as a single table generated from pipeline metadata.
