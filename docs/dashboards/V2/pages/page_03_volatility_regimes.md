...
> Detect and visualise volatility regimes (calm/normal/stressed) using rolling volatility and regime bands.
...
1. Is volatility rising or falling compared to the recent baseline?
2. Are we currently in a stressed regime?
3. Do regime shifts correspond to changes in correlation or forecast error?
...
|------|------|--------|---------|----------|
| mart.volatility_regimes | daily+intraday | Curated mart | hourly (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.volatility_regimes | daily+intraday | Curated mart | hourly (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.volatility_regimes | run | Curated mart | hourly (session) + daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
...
|-----|------------|-------------|------------------|------|
| Vol (20) | Rolling realised volatility | std(r_20)*sqrt(20) | Warn if above threshold | Configured in thresholds.yaml |
| Regime | Categorical regime label | bucket(vol) | Info | Calm/Normal/Stressed |
| Vol Δ (WoW) | Vol change vs last week | vol_now - vol_5d_ago | Info | Daily series preferred for WoW |
...
- **Measure 01:** `m_realised_vol_n`  
  - **Definition:** Rolling vol over N bars  
  - **Implementation:** SQL/Python  
  - **Formula:**  
    ```text
    vol_n = std(r_{t-n+1..t})*sqrt(n)
    ```

- **Measure 02:** `m_regime_label`  
  - **Definition:** Regime classification rule  
  - **Implementation:** SQL/Python  
  - **Formula:**  
    ```text
    regime = case when vol<p50 then Calm when vol<p90 then Normal else Stressed end
    ```
...
|-----------|------|-------|----------|---------|------------|
| V1 | Line | Rolling Volatility + Regime Bands | mart.volatility_regimes | Regime visibility | Hover + toggle bands |
| V2 | Area | Regime Timeline | mart.volatility_regimes | Shift detection | Brush to select |
| V3 | Table | Regime Summary | mart.volatility_regimes | Counts + durations | Sort |
...
- **Cross-page filter behaviour:** Cross-page filters: enabled (date range + regime selection)
...
**Refresh cadence:** hourly (session) + daily close  
**Expected runtime:** < 60s  
**Freshness SLA:** 1 hour  
**Latency notes:** Regimes are descriptive; not predictive by themselves
...
**Regimes answer “what world are we in?”** Calm vs stressed regimes change how forecasts and risk should be interpreted.
Show regime labels with clear thresholds and avoid overclaiming causality.

---

## 18) Data dictionary excerpt (minimum viable schema)
> This excerpt is the **minimum** required to render V2-P03. Full contracts live in `DATA_CONTRACTS.md`.

| Field | Type | Example | Rules / Notes |
|------|------|---------|---------------|
| date | date | 2026-02-16 | daily grain |
| realised_vol | float | 0.92 | % |
| regime_label | string | Normal | Calm/Normal/Stressed |
...
- Regime tooltip: date, realised_vol, regime_label, thresholds, trailing window.
...
- Short history → unstable thresholds; require minimum lookback for regime classification.

---

## 21) Validation & reconciliation tests (must pass before 4K export)
- Regime label must match thresholds; test on sample dates.

---

## 22) Implementation notes (Power BI / Python / Streamlit)
- Keep regime logic in a single shared function/table to avoid drift across pages.
