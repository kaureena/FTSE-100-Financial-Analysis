...
primary_grain: daily (sector returns)
...
> Summarise sector leadership/laggards using UK market sectors (where constituents mapping is available).
...
1. Which sectors are leading/lagging over selected window?
2. Is performance broad-based or concentrated?
3. Do sector patterns align with macro/FX overlays?
...
|------|------|--------|---------|----------|
| mart.sector_rotation | daily | Curated mart | daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.sector_rotation | daily | Curated mart | daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
| mart.sector_rotation | daily | Curated mart | daily close | [DATA_CONTRACTS.md#v2-marts](../../../../DATA_CONTRACTS.md#v2-marts) |
...
|-----|------------|-------------|------------------|------|
| Top Sector | Best performing sector | argmax(sector_return) | Info | Based on window |
| Sector Dispersion | Return dispersion across sectors | std(sector_returns) | Warn if very high | High dispersion implies rotation |
| Breadth | Share of sectors positive | count(sector_ret>0)/N | Info | Simple breadth proxy |
...
- **Measure 01:** `m_sector_return`  
  - **Definition:** Sector return over window  
  - **Implementation:** SQL/Python  
  - **Formula:**  
    ```text
    (last/first - 1) per sector
    ```

- **Measure 02:** `m_dispersion`  
  - **Definition:** Dispersion of sector returns  
  - **Implementation:** SQL/Python  
  - **Formula:**  
    ```text
    std(sector_returns)
...
|-----------|------|-------|----------|---------|------------|
| V1 | Treemap | Sector Performance Treemap | mart.sector_rotation | Leadership at a glance | Hover |
| V2 | Bar | Sector Returns (Ranked) | mart.sector_rotation | Rank view | Sort |
| V3 | Line | Sector Rotation Timeline | mart.sector_rotation | Dynamics | Brush |
...
- **Recommended drillthrough targets:**
  - V2-P07 Top Movers Watchlist
  - V2-P01 Footsie Pulse — Overview
- **Cross-page filter behaviour:** Cross-page filters: enabled (sector selection)
...
**Latency notes:** If sector mapping missing, fall back to constituent-level buckets
...
- `docs/dashboards/V2/exports/v2_page_06_sector_rotation.png`
...
**Sector rotation answers:** which sectors are leading/lagging and whether leadership is stable.
**How to read:**
- Sector relative strength (vs FTSE benchmark).
- Dispersion KPI indicates whether the market move is broad-based or concentrated.
- Use a rotation scatter: (momentum vs volatility) for a premium institutional feel.

---

## 18) Data dictionary excerpt (minimum viable schema)
> This excerpt is the **minimum** required to render V2-P06. Full contracts live in `DATA_CONTRACTS.md`.

| Field | Type | Example | Rules / Notes |
|------|------|---------|---------------|
| as_of_date | date | 2026-02-16 | Date |
| sector | string | Financials | ICB/GICS sector label |
| sector_return_pct | float | 0.55 | Return over selected period |
| benchmark_return_pct | float | 0.27 | FTSE benchmark return |
| relative_strength | float | 0.28 | sector_return - benchmark_return |
| sector_weight_pct | float | 14.2 | Index weight (if available) |
...
- Sector tooltip: sector, return, benchmark return, relative strength, weight, date window.
...
- Sector mapping changes (reclassification) → version sector taxonomy and note changes in release notes.
- Missing weights → rotation still works but remove weight-based narratives.

---

## 21) Validation & reconciliation tests (must pass before 4K export)
- relative_strength == sector_return - benchmark_return.
- Sector dispersion KPI == std(sector_return_pct) over selected window.

---

## 22) Implementation notes (Power BI / Python / Streamlit)
- Keep a dedicated `dim_sector` mapping table with effective dates.
- Use consistent sector colours (muted palette) and reserve neon accents for selected/alert states.
